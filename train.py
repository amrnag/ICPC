...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        batch_img_size = np.random.choice([224, 208, 192, 176, 160, 144, 128, 112, 96, 80, 64], 1)
        resize_fn = transforms.Resize(int(batch_img_size[0]))
        input = resize_fn(input)
...
def validate(
    ...
    test_image_sizes = [128, 144, 160, 176, 192, 208, 224]
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                for size in test_image_sizes:
                    ...
                    input = resize_fn(orig_input.clone())
                    output = model(input)
                    if isinstance(output, (tuple, list)):
                        output = output[0]
                    confidence, predicted = F.softmax(output, dim=1).max(1)
                    if confidence >= 0.9:
                        break
