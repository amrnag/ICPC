...
'''def collate_with_padding(batch):
    ...
    tensor = torch.zeros((batch_size, 3, 224, 224), dtype=torch.uint8)

    num_patches = int(224 / 16)
    attention_mask = torch.zeros((batch_size, 1, num_patches*num_patches+1))
    
    for i in range(batch_size):
        size = batch[i][0].shape
        #### Check here
        h_offset = int(num_patches - (size[1] / 16))
        w_offset = int(num_patches - (size[2] / 16))
        for j in range(1, num_patches + 1):
            attention_mask[i, :, j*num_patches+1-w_offset:j*num_patches+1] = 1
        attention_mask[i, :, (num_patches*(num_patches-h_offset))+1:] = 1
        ####
        tensor[i] += F.pad(torch.from_numpy(batch[i][0]), (0, 0, 224 - size[2], 224 - size[1]))
    return tensor, targets, attention_mask'''
