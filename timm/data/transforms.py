...
class RatioPreserveResize:
    ...
    def get_params(img):
        source_size = img.size[::-1]  # h, w
        scaling_factor = max(source_size) / 224
        size = [(x / scaling_factor) for x in source_size]
        size = [int(16 * np.floor(x / 16)) for x in size]
        '''ratio = source_size[0] / source_size[1]
        scaling_factor = 16 * 14 / (source_size[1] * np.sqrt(ratio))
        size = [(scaling_factor * x) for x in source_size]
        size = [int(16 * np.floor(x / 16)) for x in size]'''
    ...
    def __call__(self, img):
        ...
        img = F.resize(img, size, self.interpolation)
        #img = F.pad(img, (0, 0, 224 - size[1], 224 - size[0]))
