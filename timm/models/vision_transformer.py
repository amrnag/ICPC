...
class VisionTransformer(nn.Module):
    ...
    def _pos_embed(self, x, h_offset=0, w_offset=0):
        if self.no_embed_class:
            ...
            num_patches = int(np.sqrt(self.pos_embed.shape[1]))
            pos_embed_mask = torch.ones((1, num_patches*num_patches+1)).bool()
            for i in range(1, num_patches + 1):
                pos_embed_mask[:, i*num_patches+1-w_offset:i*num_patches+1] = False
            pos_embed_mask[:, (num_patches*(num_patches-h_offset))+1:] = False
            pos_embed = self.pos_embed[pos_embed_mask]
            x = x + pos_embed
    ...
    def forward_features(self, x):
        ...
        num_patches = int(np.sqrt(self.pos_embed.shape[1]))
        h_offset = int(num_patches - (size[2] / 16))
        w_offset = int(num_patches - (size[3] / 16))
        x = self._pos_embed(x, h_offset, w_offset)
